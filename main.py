...
try:
	victim   = str(Write.Input("[?] Victim Username: ", Colors.purple_to_blue, interval=0))
	amount   = int(Write.Input("[?] Report Amount (0=inf): ", Colors.purple_to_blue, interval=0))
	reason   = int(Write.Input("[?] Reason for Report (1-9): ", Colors.purple_to_blue, interval=0))
	cooldown = int(Write.Input("[?] Cooldown: ", Colors.purple_to_blue, interval=0))
except Exception as E:
	Write.Print(f"\n[>] Error: {E}", Colors.purple_to_red, interval=0)
	Write.Print("\n[>] Enter to Exit...", Colors.purple_to_red, interval=0)
	input()
	exit()
